  ...
  Drupal.behaviors.adminToolbarSearch = {
    ...
    attach: function (context) {
      ...
      $(elements).each(function () {
        ...
        $searchInput
          .autocomplete({
            ...
            source: function (request, response) {
              ...
              if (
                !$self.extraFetched &&
                drupalSettings.adminToolbarSearch.loadExtraLinks
              ) {
                $.getJSON(
                  Drupal.url("admin/admin-toolbar-search"),
                  function (data) {
                    $(data).each(function () {
                      var item = this;
            ...
            open: function () {
              var zIndex =
                $("#toolbar-item-administration-tray").css("z-index") + 1;
          ...
          return $("<li>")
            .append(
              '<div ><a href="' +
                ...
                item.labelRaw +
                ' <span class="admin-toolbar-search-url">' +
                item.value +
                "</span></a></div>"
            )
            .appendTo(ul);
        ...
        $("#admin-toolbar-mobile-search-tab .toolbar-item", context).click(
          function (e) {
            e.preventDefault();
      ...
      $(item)
        ...
        .each(function () {
          if ($(this).hasClass("menu-item")) {
            ...
            if (
              $link.length &&
              !$link.hasClass("admin-toolbar-search-ignore")
            ) {
