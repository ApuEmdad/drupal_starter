(function ($, Drupal, drupalSettings) {
  ...
  Drupal.behaviors.tokenTree = {
    attach: function (context, settings) {
      $(once("token-tree", "table.token-tree", context)).treetable({
        expandable: true,
      });
    },
  };
      ...
      once(
        ...
        var newThis = $(
          '<a href="javascript:void(0);" title="' +
            Drupal.t("Insert this token into your form") +
            '">' +
            $(token).html() +
            "</a>"
        ).click(function () {
          ...
          // Direct CodeMirror support.
            ...
            drupalSettings.tokenFocusedField &&
            $(drupalSettings.tokenFocusedField).parents(".CodeMirror").length
          ) {
            var editor = $(drupalSettings.tokenFocusedField).parents(
              ".CodeMirror"
            )[0].CodeMirror;
